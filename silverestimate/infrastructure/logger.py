...
class DatabaseOperation:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if issubclass(exc_type, sqlite3.Error):
            self.logger.error(
                f"Database error during {self.operation_name}: {str(exc_val)}",
                exc_info=True,
            )
        elif issubclass(exc_type, ValueError):
            self.logger.warning(
                f"Value error during {self.operation_name}: {str(exc_val)}",
                exc_info=True,
            )
        else:
            self.logger.error(
                f"Unexpected error during {self.operation_name}: {str(exc_val)}",
                exc_info=True,
            )
...
def cleanup_old_logs(log_dir="logs", max_age_days=1):
    ...
    logger.debug(
        f"Starting log cleanup for files older than {max_age_days} days in {log_dir}"
    )

    # Validate parameters
    if max_age_days < 1:
        logger.warning(
            f"Invalid max_age_days value ({max_age_days}), using default of 1 day"
        )
        max_age_days = 1
            ...
            if file_path.is_file():
                ...
                if file_time < cutoff:
                    try:
                        ...
                        logger.warning(
                            f"Failed to remove old archived log file {file_path}: {e}"
                        )

    logger.info(
        f"Log cleanup completed: removed {removed_count} files older than {max_age_days} days"
    )
...
class LogCleanupScheduler:
    ...
    def _run_cleanup(self):
        ...
        try:
            self.logger.debug(
                f"Running scheduled cleanup for logs older than {self.cleanup_days} days"
            )
            removed_count = cleanup_old_logs(self.log_dir, self.cleanup_days)
            self.logger.info(
                f"Automatic log cleanup completed. Removed {removed_count} old log files"
            )
            ...
            self.logger.debug(
                f"Next scheduled cleanup: {next_cleanup.strftime('%Y-%m-%d %H:%M:%S')}"
            )
    ...
    if config["auto_cleanup"]:
        if _cleanup_scheduler is None:
            ...
            _cleanup_scheduler = LogCleanupScheduler(
                log_dir=config["log_dir"], cleanup_days=config["cleanup_days"]
            )
