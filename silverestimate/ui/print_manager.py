...
class PrintManager:
    ...
    def _generate_estimate_old_format(self, estimate_data):
        ...
        def format_line(*args):
            ...
            try:
                ...
                poly = f"{args[4]:>{W_POLY}.0f}"  # Poly as integer
                ...
                logging.getLogger(__name__).error(
                    f"Error formatting line: {e}, Data: {args}"
                )
                return " " * TOTAL_WIDTH
        ...
        reg_f, reg_w, reg_g, reg_p = 0.0, 0.0, 0.0, 0.0
        sb_f, sb_w, sb_g, sb_p = 0.0, 0.0, 0.0, 0.0
        ret_gf, ret_gw, ret_gg, ret_gp = 0.0, 0.0, 0.0, 0.0
        ret_sf, ret_sw, ret_sg, ret_sp = 0.0, 0.0, 0.0, 0.0
    ...
    def _generate_estimate_new_format(self, estimate_data):
        ...
        if regular_items:
            ...
            output.append(
                format_line(
                    "", "TOTAL", reg_g, reg_p, reg_n, None, None, None, reg_f, reg_w
                )
            )
            output.append(sep_eq)
        ...
        if silver_bar_items:
            ...
            output.append(sep_dash)
            output.append(
                format_line("", "TOTAL", sb_g, sb_p, sb_n, None, None, None, sb_f, sb_w)
            )
            output.append(sep_eq)
    ...
    def _generate_estimate_thermal_format(self, estimate_data):
        ...
        if regular_items:
            ...
            output.append(sep)
            output.append(
                f"TOTAL G:{reg_g:9.2f} P:{reg_p:9.2f} N:{reg_n:9.2f}"[
                    :TOTAL_WIDTH
                ].ljust(TOTAL_WIDTH)
            )
        ...
        if silver_bar_items:
            ...
            output.append(sep)
            output.append(
                f"TOTAL G:{sb_g:9.2f} P:{sb_p:9.2f} N:{sb_n:9.2f}"[:TOTAL_WIDTH].ljust(
                    TOTAL_WIDTH
                )
            )
            line = f"      Fi:{sb_f:9.2f}"
        ...
        if return_goods:
            ...
            output.append(sep)
            output.append(
                f"TOTAL G:{ret_gg:9.2f} P:{ret_gp:9.2f} N:{ret_gn:9.2f}"[
                    :TOTAL_WIDTH
                ].ljust(TOTAL_WIDTH)
            )
        ...
        if return_silver_bars:
            ...
            output.append(sep)
            output.append(
                f"TOTAL G:{ret_sg:9.2f} P:{ret_sp:9.2f} N:{ret_sn:9.2f}"[
                    :TOTAL_WIDTH
                ].ljust(TOTAL_WIDTH)
            )
    ...
    def _open_preview_with_enhancements(
        self, html_content, parent_widget, title, table_mode=False
    ):
        ...
        preview.paintRequested.connect(
            lambda printer: self._print_html(
                printer, html_content, table_mode=table_mode
            )
        )
        ...
        try:
            ...
            if preview_widget:
                ...
                logging.getLogger(__name__).debug(
                    f"Applying zoom factor: {zoom_factor}"
                )
                ...
                logging.getLogger(__name__).warning(
                    "Could not find QPrintPreviewWidget to set zoom."
                )
        except Exception as zoom_err:
            logging.getLogger(__name__).warning(
                f"Error setting initial zoom: {zoom_err}"
            )

        # Enhance toolbar
        try:
            self._augment_preview_toolbar(
                preview, preview_widget, html_content, table_mode, parent_widget
            )
        except Exception as tb_err:
            logging.getLogger(__name__).warning(
                f"Could not augment preview toolbar: {tb_err}"
            )
        ...
        try:
            ...
            logging.getLogger(__name__).warning(
                f"Could not save preview zoom: {save_err}"
            )

    def _augment_preview_toolbar(
        self, preview, preview_widget, html_content, table_mode, parent_widget
    ):
        ...
        act_pdf.triggered.connect(
            lambda: self._export_pdf_via_dialog(html_content, table_mode, parent_widget)
        )
        ...
        act_page.triggered.connect(
            lambda: self._page_setup_and_refresh(preview, html_content, table_mode)
        )
        ...
        act_orient.triggered.connect(
            lambda: self._toggle_orientation_and_refresh(
                preview, html_content, table_mode
            )
        )
        ...
        act_qprint.triggered.connect(
            lambda: self._quick_print_current(
                preview, html_content, table_mode, parent_widget
            )
        )
    ...
    def _export_pdf_via_dialog(self, html_content, table_mode, parent_widget):
        ...
        try:
            ...
            settings = get_app_settings()
            margins_str = settings.value(
                "print/margins", defaultValue="10,5,10,5", type=str
            )
            try:
                margins = [int(m.strip()) for m in margins_str.split(",")]
                if len(margins) != 4:
            ...
            QMessageBox.information(
                parent_widget, "Saved", f"PDF saved to:\n{file_path}"
            )
        except Exception as e:
            QMessageBox.critical(
                parent_widget, "Export Failed", f"Could not export PDF:\n{str(e)}"
            )
    ...
    def _quick_print_current(self, preview, html_content, table_mode, parent_widget):
        ...
        try:
            ...
            QMessageBox.information(
                parent_widget or preview, "Printing", "Document sent to printer."
            )
        except Exception as e:
            QMessageBox.critical(
