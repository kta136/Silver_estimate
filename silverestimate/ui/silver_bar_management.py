...
class SilverBarDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.auto_refresh_checkbox.toggled.connect(
            lambda checked: self._toggle_auto_refresh(checked)
        )
        ...
        percent_lbl.setStyleSheet(
            "font-size: 12px; margin-left: 2px; margin-right: 12px;"
        )
        ...
        """)
        try:
            self.available_bars_table.verticalHeader().setDefaultSectionSize(
                28
        ...
        self.available_totals_label = QLabel(
            "Available Bars: 0 | Total Weight: 0.000 g | Total Fine Wt: 0.000 g"
        )
        ...
        """)
        self.add_to_list_button.setToolTip(
            "Add selected 'Available' bars to the selected list."
        )
        ...
        self.add_all_button.setToolTip(
            "Add all available (filtered) bars to the selected list."
        )
        ...
        """)
        self.remove_from_list_button.setToolTip(
            "Remove selected bars from the list (status becomes 'In Stock')."
        )
        ...
        """)
        self.remove_all_button.setToolTip(
            "Remove all bars from the selected list (status becomes 'In Stock')."
        )
        ...
        """)
        self.generate_optimal_button.setToolTip(
            "Generate an optimal list for a target fine weight"
        )
        ...
        """)
        try:
            self.list_bars_table.verticalHeader().setDefaultSectionSize(
                28
            )  # Increased row height for better readability
        except Exception:
            pass
        ...
        self.list_totals_label = QLabel(
            "List Bars: 0 | Total Weight: 0.000 g | Total Fine Wt: 0.000 g"
        )
        ...
        try:
            QShortcut(
                QKeySequence.Refresh, self, activated=self.load_available_bars
            )  # F5 / Ctrl+R
        ...
        try:
            self.available_bars_table.selectionModel().selectionChanged.connect(
                self._on_selection_changed
            )
            self.list_bars_table.selectionModel().selectionChanged.connect(
                self._on_selection_changed
            )
        except Exception:
            pass
        ...
        try:
            self.available_bars_table.horizontalHeader().sortIndicatorChanged.connect(
                lambda col, order: self._save_table_sort_state(
    ...
    def create_new_list(self):
        ...
        note, ok = QInputDialog.getText(
            self, "Create New List", "Enter a note for the new list:", QLineEdit.Normal
        )
        if ok:
    ...
    def _create_list_from_selection(self):
        ...
        if not selected:
            QMessageBox.warning(
                self, "Selection", "Select one or more available bars first."
            )
            return
        note, ok = QInputDialog.getText(
            self,
        ...
        if added_count:
            QMessageBox.information(
                self, "Success", f"Created list and added {added_count} bar(s)."
            )
        if failed:
            QMessageBox.warning(
                self, "Partial", f"Failed to add bars: {', '.join(failed)}"
            )
    ...
    def add_selected_to_list(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            if added_count > 0:
                QMessageBox.information(
                    self, "Success", f"{added_count} bar(s) added to the list."
                )
            if failed_ids:
                QMessageBox.warning(
                    self,
    ...
    def remove_selected_from_list(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            if removed_count > 0:
                QMessageBox.information(
                    self, "Success", f"{removed_count} bar(s) removed from the list."
                )
            if failed_ids:
                QMessageBox.warning(
                    self,
    ...
    def edit_list_note(self):
        ...
        if not details:
            QMessageBox.warning(
                self, "Error", "Could not retrieve current list details."
            )
            return
        ...
        if ok and new_note != current_note:
            if self.db_manager.update_silver_bar_list_note(
                ...
                self.list_details_label.setText(
    ...
    def delete_selected_list(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            if success:
                QMessageBox.information(
                    self, "Success", f"List '{list_name}' deleted successfully."
                )
    ...
    def mark_list_as_issued(self):
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                QMessageBox.critical(
                    self, "Error", f"Failed to mark list as issued: {e}"
                )
    ...
    def print_selected_list(self):
        ...
        if not details:
            QMessageBox.warning(
                self, "Error", "Could not retrieve list details for printing."
            )
            return
        ...
        logging.getLogger(__name__).info(
            f"Printing list {details['list_identifier']} (ID: {self.current_list_id}) with {len(bars_in_list)} bars."
        )
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Error",
                f"Print function not found or incorrect in PrintManager: {ae}",
            )
        except Exception as e:
            QMessageBox.critical(
                self,
                "Print Error",
                f"An unexpected error occurred during printing: {e}\n{traceback.format_exc()}",
            )
    ...
    def generate_optimal_list(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            try:
                ...
                selected_bars = self._find_optimal_combination(
                ...
                QMessageBox.critical(
                    self,
                    "Error",
                    f"Failed to generate optimal list: {e}\n{traceback.format_exc()}",
                )

    def _find_optimal_combination(
        self, available_bars, min_target, max_target, optimization_type
    ):
        ...
        try:
            ...
            try:
                ...
                if isinstance(ar, (bool, str)):
                    checked = (
                        bool(ar) if isinstance(ar, bool) else (ar.lower() == "true")
                    )
    ...
    def _update_transfer_buttons_state(self):
        try:
            ...
            try:
                has_list_sel = bool(
                    self.list_bars_table.selectionModel().selectedRows()
                )
            except Exception:
                pass
    ...
    def add_all_filtered_to_list(self):
        ...
        if added:
            QMessageBox.information(
                self, "Success", f"{added} bar(s) added to the list."
            )
        if failed:
            QMessageBox.warning(
                self, "Partial", f"Failed to add bars: {', '.join(failed)}"
    ...
    def remove_all_from_list(self):
        ...
        if removed:
            QMessageBox.information(
                self, "Success", f"{removed} bar(s) removed from the list."
            )
        if failed:
            QMessageBox.warning(
                self, "Partial", f"Failed to remove bars: {', '.join(failed)}"
            )
    ...
    def export_current_list_to_csv(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Export List to CSV", "silver_bars.csv", "CSV Files (*.csv)"
        )
        if not path:
            return
...
class OptimalListDialog(QDialog):
    ...
    def init_ui(self):
        ...
        # Add explanation
        explanation = QLabel(
            "• Minimum bars: Find the smallest number of bars within the weight range\n• Maximum bars: Use as many bars as possible within the weight range"
        )
    ...
    class MockDBManager:
        def get_silver_bar_lists(self):
            return [
                {
                    "list_id": 1,
                    "list_identifier": "L-20250424-001",
                    "creation_date": "2025-04-24 10:00:00",
                    "list_note": "Test List 1",
                },
                {
                    "list_id": 2,
                    "list_identifier": "L-20250424-002",
                    "creation_date": "2025-04-24 11:00:00",
                    "list_note": None,
                },
            ]
        ...
        def get_silver_bars(
            ...
            bars = [
                ...
                {
                    "bar_id": 103,
                    "estimate_voucher_no": "202504232",
                    "weight": 1000.123,
                    "purity": 99.0,
                    "fine_weight": 990.12177,
                    "date_added": "2025-04-23",
                    "status": "In Stock",
                    "list_id": None,
                },
        ...
        def get_silver_bar_list_details(self, list_id):
            return (
                {
                    "list_id": list_id,
                    "list_identifier": f"L-20250424-{list_id:03d}",
                    "creation_date": "...",
                    "list_note": "Mock Note",
                }
                if list_id
        ...
        def create_silver_bar_list(self, note):
            print(f"Mock: Create list with note: {note}")
            return 3  # Simulate new ID
