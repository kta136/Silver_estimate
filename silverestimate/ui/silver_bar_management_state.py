...
class SilverBarManagementStateStore:
    ...
    def _save_table_sort_state(self, which, table) -> None:
        try:
            ...
            self.host.logger.debug(
                "Could not persist %s table sort state: %s", which, exc
            )
    ...
    def _save_ui_state(self) -> None:
        try:
            ...
            if hasattr(self.host, "_splitter"):
                settings.set(
                    SettingsKey.UI_SILVER_BARS_SPLITTER,
                    self.host._splitter.saveState(),
                )
            settings.set(
                SettingsKey.UI_SILVER_BARS_AVAILABLE_COLUMNS,
                self._get_table_column_widths(self.host.available_bars_table),
            )
            settings.set(
                SettingsKey.UI_SILVER_BARS_LIST_COLUMNS,
                self._get_table_column_widths(self.host.list_bars_table),
            )
            self._save_table_sort_state("available", self.host.available_bars_table)
            self._save_table_sort_state("list", self.host.list_bars_table)
            settings.set(
                SettingsKey.UI_SILVER_BARS_WEIGHT_QUERY,
                self.host.weight_search_edit.text(),
            )
            settings.set(
            ...
            settings.set(
                ...
                self.host.date_range_combo.currentText(),
            )
            settings.sync()
        except Exception as exc:
            self.host.logger.debug(
    ...
    def _restore_ui_state(self) -> None:
        try:
            ...
            if (
                isinstance(geometry, (QByteArray, bytes, bytearray, memoryview))
                and geometry
            ):
            ...
            if (
                isinstance(state, (QByteArray, bytes, bytearray, memoryview))
                and state
                and hasattr(self.host, "_splitter")
            ):
                self.host._splitter.restoreState(state)
            if hasattr(self.host, "_splitter"):
                self.host._splitter.setOrientation(Qt.Orientation.Horizontal)

            if settings.contains(SettingsKey.UI_SILVER_BARS_DATE_RANGE):
                dr = settings.get_text(SettingsKey.UI_SILVER_BARS_DATE_RANGE)
                idx = self.host.date_range_combo.findText(dr)
                if idx >= 0:
                    self.host.date_range_combo.setCurrentIndex(idx)

            weight_query = settings.get_text(SettingsKey.UI_SILVER_BARS_WEIGHT_QUERY)
            self.host.weight_search_edit.setText(weight_query)
            ...
            if settings.contains(
                ...
                self.host.available_bars_table.sortByColumn(
                    av_col, Qt.SortOrder(av_ord)
                )
            if settings.contains(
                ...
                self.host.list_bars_table.sortByColumn(ls_col, Qt.SortOrder(ls_ord))
    ...
    def _restore_selected_list_from_settings(self) -> None:
        try:
            ...
            idx = self.host.list_combo.findData(saved_id_int)
            if idx >= 0:
                self.host.list_combo.setCurrentIndex(idx)
        except Exception as exc:
            self.host.logger.debug(
                "Could not restore selected silver bar list: %s", exc
            )

    def _get_table_column_widths(self, table) -> Any:
        try:
            header = table.horizontalHeader()
            return [header.sectionSize(i) for i in range(table.model().columnCount())]
        except Exception as exc:
            self.host.logger.debug("Could not capture table column widths: %s", exc)
            return None

    def _apply_table_column_widths(self, table, widths) -> None:
        try:
            if not widths:
                return
            header = table.horizontalHeader()
            ...
            self.host.logger.debug("Could not apply stored table widths: %s", exc)
    ...
    def _restore_table_column_widths(self) -> None:
        try:
            ...
            self._apply_table_column_widths(self.host.available_bars_table, available)
            self._apply_table_column_widths(self.host.list_bars_table, list_cols)
        except Exception as exc:
            self.host.logger.debug("Could not restore table column widths: %s", exc)

    def _current_date_range(self) -> Any:
        try:
            text = self.host.date_range_combo.currentText()
        except Exception as exc:
            self.host.logger.debug("Could not read date range combo value: %s", exc)
            return None
        now = datetime.now()
        if text == "Today":
    ...
    def _is_embedded(self) -> Any:
        try:
            ...
            self.host.logger.debug(
                "Could not determine embedded silver bar state: %s", exc
            )
            return False
    ...
    def _navigate_back_to_estimate(self) -> None:
        try:
            ...
            self.host.logger.debug("Could not navigate back to estimate view: %s", exc)
