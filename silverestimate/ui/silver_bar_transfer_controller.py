...
class SilverBarTransferController:
    """Handle moving bars between lists and exporting list contents."""

    def __init__(self, host: SilverBarDialog) -> None:
        self.host = host

    def _bulk_assign_to_list(self, bar_ids, list_id) -> Any:
        assign_bulk = getattr(self.host.db_manager, "assign_bars_to_list_bulk", None)
        ...
        for bar_id in list(bar_ids or []):
            if self.host.db_manager.assign_bar_to_list(bar_id, list_id):
                added += 1
            else:
                failed.append(str(bar_id))
        return added, failed

    def _bulk_remove_from_list(self, bar_ids) -> Any:
        remove_bulk = getattr(self.host.db_manager, "remove_bars_from_list_bulk", None)
        ...
        failed = []
        for bar_id in list(bar_ids or []):
            if self.host.db_manager.remove_bar_from_list(bar_id):
                removed += 1
            else:
                failed.append(str(bar_id))
    ...
    def add_selected_to_list(self) -> None:
        ...
        selected_rows = self._selected_rows(self.host.available_bars_table)
        ...
        reply = QMessageBox.question(
            self.host,
            "Confirm Add",
            f"Add {len(bar_ids_to_add)} selected bar(s) to list '{self.host.list_combo.currentText()}'?",
        ...
        added_count, failed_ids = self._run_with_wait_cursor(
            lambda: self._bulk_assign_to_list(
                bar_ids_to_add, self.host.current_list_id
            ),
    ...
    def remove_selected_from_list(self) -> None:
        ...
        selected_rows = self._selected_rows(self.host.list_bars_table)
        if not selected_rows:
            QMessageBox.warning(
                self.host,
                "Selection Error",
        ...
        bar_ids_to_remove = self._bar_ids_from_indexes(
            self.host.list_bars_table, selected_rows
        )
        ...
        reply = QMessageBox.question(
            self.host,
            "Confirm Remove",
            f"Remove {len(bar_ids_to_remove)} selected bar(s) from list '{self.host.list_combo.currentText()}'?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No,
        )
        if reply != QMessageBox.StandardButton.Yes:
    ...
    def add_all_filtered_to_list(self) -> None:
        if self.host.current_list_id is None:
            ...
            return
        row_count = self.host.available_bars_table.model().rowCount()
        if row_count == 0:
        ...
        reply = QMessageBox.question(
            self.host,
            "Confirm Add All",
            f"Add all {row_count} available bar(s) to list '{self.host.list_combo.currentText()}'?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No,
        )
        if reply != QMessageBox.StandardButton.Yes:
            return
        bar_ids = self._all_bar_ids(self.host.available_bars_table)
        added, failed = self._run_with_wait_cursor(
            lambda: self._bulk_assign_to_list(bar_ids, self.host.current_list_id),
    ...
    def remove_all_from_list(self) -> None:
        if self.host.current_list_id is None:
            ...
            return
        row_count = self.host.list_bars_table.model().rowCount()
        if row_count == 0:
        ...
        reply = QMessageBox.warning(
            self.host,
            "Confirm Remove All",
            f"Remove all {row_count} bar(s) from list '{self.host.list_combo.currentText()}'?\nThis will set their status to 'In Stock'.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.Cancel,
            QMessageBox.StandardButton.Cancel,
        )
        if reply != QMessageBox.StandardButton.Yes:
            return
        bar_ids = self._all_bar_ids(self.host.list_bars_table)
    ...
    def export_current_list_to_csv(self) -> None:
        ...
        try:
            with open(path, "w", newline="", encoding="utf-8") as handle:
                ...
                model = self.host.list_bars_table.model()
                for row in range(model.rowCount()):
                    output_row = [
                        self.host._bar_id_from_table(self.host.list_bars_table, row)
                        or ""
                    ]
                    row_payload = None
                    ...
                    for column in range(model.columnCount()):
                        if (
                            ...
                            continue
                        output_row.append(
                            self.host._table_cell_text(
                                self.host.list_bars_table, row, column
                            )
                        )
    ...
    def _bar_ids_from_indexes(self, table, indexes) -> Any:
        bar_ids = []
        for index in indexes:
            bar_id = self.host._bar_id_from_table(table, index.row())
            if bar_id is not None:
                bar_ids.append(bar_id)
        return bar_ids

    def _all_bar_ids(self, table) -> Any:
        bar_ids = []
        row_count = table.model().rowCount()
        for row in range(row_count):
            bar_id = self.host._bar_id_from_table(table, row)
            if bar_id is not None:
                bar_ids.append(bar_id)
        return bar_ids

    def _run_with_wait_cursor(
        self, operation, *, enable_log: str, restore_log: str
    ) -> Any:
        try:
            QApplication.setOverrideCursor(Qt.CursorShape.WaitCursor)
        except Exception as exc:
            self.host.logger.debug(enable_log, exc)
        try:
            return operation()
        finally:
            try:
                ...
                self.host.logger.debug(restore_log, exc)
